...
function startup(onMessage) {
    self.onmessage = function(event) {
        ...
        // Justin changed the line below:
        // var pocketsphinxWASM = (event.data && 'pocketsphinx.wasm' in event.data) ? event.data['pocketsphinx.wasm'] : '/webapp/js/pocketsphinx.wasm';
        var pocketsphinxWASM = (event.data && 'pocketsphinx.wasm' in event.data) ? event.data['pocketsphinx.wasm'] : 'https://developer.nasa.gov/pages/jgossess/pocketsphinxjs_clone/webapp/js/pocketsphinx.wasm';
        // var pocketsphinxWASM = (event.data && 'pocketsphinx.wasm' in event.data) ? event.data['pocketsphinx.wasm'] : 'https://github.com/syl22-00/pocketsphinx.js/blob/master/webapp/js/pocketsphinx.wasm?raw=true';
...
function addWords(data, clbId) {
	...
	console.log("words in addWords",data)
...
function addKeyword(data, clbId) {
    var output;
    console.log("data in addKeyword recognizer.js = ",data)
    if (recognizer) {
	if (data.length > 0) {
	    var id_v = new Module.Integers();
